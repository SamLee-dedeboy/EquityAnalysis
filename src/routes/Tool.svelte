<script>
    ...
    try {
      ...
      if (res.ok) {
        ...
        policies = policies.map(p => ({
            ...p,
            analysis_status: p.analysis_status || (p.source === 'preprocessed' ? 'completed' : 'unknown')
        }));
      } else {
        console.error("Failed to load initial policies list:", res.status, await res.text());
      }
    } catch (err) {
  ...
  async function loadPolicyData(policyId) {
    ...
    if (selectedPolicy) {
      ...
      if (selectedPolicy.source === 'preprocessed' || selectedPolicy.analysis_status === 'completed') {
        try {
          ...
          if (res.ok) {
            ...
            console.error("Failed to load full policy data:", res.status, await res.text());
            analysisStatus = 'failed';
            // Update the policy in store/local with error info
            currentPolicy.update(p => ({ ...p, analysis_status: 'failed', analysis_error: 'Failed to load report data.' }));
            currentDoc = { ...currentDoc, analysis_status: 'failed', analysis_error: 'Failed to load report data.' };
          }
        } catch (err) {
          console.error("Network error loading full policy data:", err);
          analysisStatus = 'failed';
          currentPolicy.update(p => ({ ...p, analysis_status: 'failed', analysis_error: 'Network error loading report data.' }));
          currentDoc = { ...currentDoc, analysis_status: 'failed', analysis_error: 'Network error loading report data.' };
        }
      } else if (selectedPolicy.source === 'user' && selectedPolicy.analysis_status !== 'failed') {
        ...
        pollAnalysisStatus(currentSessionId); // Initial immediate poll to display current status
          ...
          analysisStatus = 'failed';
          currentPolicy.update(p => ({ ...p, analysis_status: 'failed', analysis_error: selectedPolicy.analysis_error || 'Analysis previously failed.' }));
          currentDoc = { ...currentDoc, analysis_status: 'failed', analysis_error: selectedPolicy.analysis_error || 'Analysis previously failed.' };
      }
    } else {
  ...
  async function handleFileUpload(event) {
    ...
    const tempPolicyEntry = {
        id: tempId,
        document: { title: file.name, filename: file.name, size_kb: Math.round(file.size / 1024), upload_date_utc: new Date().toISOString() },
        source: "user",
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        policies = policies.map(p => p.id === tempId ? { ...p, id: currentSessionId, analysis_status: analysisStatus } : p);
        ...
        currentDoc = { ...currentDoc, id: currentSessionId, analysis_status: analysisStatus };
      ...
      analysisStatus = 'failed';
      policies = policies.map(p => p.id === tempId ? { ...p, analysis_status: 'failed', analysis_error: 'Network error during upload.' } : p);
      currentPolicy.update(p => ({ ...p, analysis_status: 'failed', analysis_error: 'Network error during upload.' }));
      currentDoc = { ...currentDoc, analysis_status: 'failed', analysis_error: 'Network error during upload.' };
    } finally {
  ...
  async function pollAnalysisStatus(sessionId) {
    ...
    try {
        const response = await fetch(`http://localhost:8000/get_analysis_status/${sessionId}`);
        if (!response.ok) {
            ...
            policies = policies.map(p => p.id === sessionId ? { ...p, analysis_status: 'failed', analysis_error: 'Failed to get status.' } : p);
            currentPolicy.update(p => ({ ...p, analysis_status: 'failed', analysis_error: 'Failed to get status.' })); // Also update currentPolicy
            currentDoc = { ...currentDoc, analysis_status: 'failed', analysis_error: 'Failed to get status.' }; // Update local for ReportView
            return;
        }
        ...
        policies = policies.map(p => p.id === sessionId ? { ...p, analysis_status: newStatus, analysis_error: result.analysis_error } : p);
        currentPolicy.update(p => ({ ...p, analysis_status: newStatus, analysis_error: result.analysis_error })); // Update currentPolicy store
        currentDoc = { ...currentDoc, analysis_status: newStatus, analysis_error: result.analysis_error }; // Update local for ReportView
        ...
        if (newStatus === "completed") {
            ...
            clearInterval(analysisPollingTimer);
            analysisPollingTimer = null;
            console.error(`Analysis for ${sessionId} failed: ${result.analysis_error}`);
        }
        ...
        clearInterval(analysisPollingTimer);
        analysisPollingTimer = null;
        policies = policies.map(p => p.id === sessionId ? { ...p, analysis_status: 'failed', analysis_error: 'Network error.' } : p);
        currentPolicy.update(p => ({ ...p, analysis_status: 'failed', analysis_error: 'Network error.' }));
        currentDoc = { ...currentDoc, analysis_status: 'failed', analysis_error: 'Network error.' };
    }
  }
  ...
  async function displayAnalysisResult(sessionId) {
    ...
    try {
        const response = await fetch(`http://localhost:8000/get_analysis_result/${sessionId}`);
        if (!response.ok) {
            console.error("Failed to retrieve analysis result:", response.status, await response.text());
            analysisResultFetched = false; // Allow re-attempt if the fetch itself fails
            analysisStatus = 'failed';
            currentPolicy.update(p => ({ ...p, analysis_status: 'failed', analysis_error: 'Failed to retrieve analysis report data.' }));
            currentDoc = { ...currentDoc, analysis_status: 'failed', analysis_error: 'Failed to retrieve analysis report data.' };
            return;
        }
        ...
        if (result.analysis_status === "completed") {
            ...
            policies = policies.map(p => {
                if (p.id === sessionId) {
                    return { ...p, ...result.analysis_data, analysis_status: 'completed' }; // Merge full data into the existing policy object
                }
                return p;
            });
            ...
            console.error("Analysis result was not 'completed' when fetched via get_analysis_result:", result.analysis_status);
            analysisResultFetched = false;
            analysisStatus = 'failed';
            currentPolicy.update(p => ({ ...p, analysis_status: 'failed', analysis_error: 'Unexpected status when retrieving full report.' }));
            currentDoc = { ...currentDoc, analysis_status: 'failed', analysis_error: 'Unexpected status when retrieving full report.' };
        }
    } catch (error) {
        console.error("Error fetching analysis result:", error);
        analysisResultFetched = false;
        analysisStatus = 'failed';
        currentPolicy.update(p => ({ ...p, analysis_status: 'failed', analysis_error: 'Network error while fetching analysis report.' }));
        currentDoc = { ...currentDoc, analysis_status: 'failed', analysis_error: 'Network error while fetching analysis report.' };
    }
  }
        ...
        <ul style="list-style: none; padding: 0; margin: 0;">
          {#each policies as policy (policy.id)}
            ...
            >
              ...
              >
                {policy.document?.title || policy.id} <!-- Display title, fallback to ID -->
                {#if policy.source === 'user'}
                  <span
                    class="analysis-status-dot"
                    class:status-pending={policy.analysis_status === 'pending' || policy.analysis_status === 'waiting_vs_processing' || policy.analysis_status === 'analysis_generating'}
                    class:status-completed={policy.analysis_status === 'completed'}
                    class:status-failed={policy.analysis_status === 'failed'}
                    title="Analysis Status: {policy.analysis_status?.replace(/_/g, ' ') || 'Unknown'}"
    ...
    <div class="report-chat-container">
      ...
      <div class="report-panel">
        <div class="report-content">
          ...
          <ReportView currentDoc={currentDoc} analysisStatus={analysisStatus} />
        </div>
      </div>
      ...
      {#if chatPanel}
        <div class="chat-panel" in:slide={{ axis: "x" }}>
          <ChatPanel
            ...
            analysisIsGenerating={analysisStatus !== 'completed' && analysisStatus !== 'failed' && currentSessionId !== null}
